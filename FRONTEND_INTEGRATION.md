...
**Payment finalize** (`PUT /payments/{id}`, the only client-facing payment
...
it in the normal flow; use `PUT /payments/{id}` after the order is completed.
...
1. `POST /orders` — waiter opens the order once the reservation is `CONFIRMED`
   ...
   auto-created as `PENDING`, amount `0`.
2. `POST /order-items` — called once per dish/round (repeatable, any number of
   times) while the order is `IN_PROGRESS`. Each call bumps `order.totalAmount`
   and the linked payment's `amount` together automatically. Use
   `PUT`/`DELETE /order-items/{id}` to adjust quantity or remove a line.
